# Autogenerated wrapper script for MUMPS_jll for x86_64-apple-darwin14-libgfortran5
export libdmumps, libzmumps, libsmumps, libcmumps
...
const libdmumps_splitpath = ["lib", "libdmumps.dylib"]
...
const libdmumps = "@rpath/libdmumps.dylib"


# Relative path to `libzmumps`
const libzmumps_splitpath = ["lib", "libzmumps.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libzmumps = "@rpath/libzmumps.dylib"


# Relative path to `libsmumps`
const libsmumps_splitpath = ["lib", "libsmumps.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libsmumps = "@rpath/libsmumps.dylib"


# Relative path to `libcmumps`
const libcmumps_splitpath = ["lib", "libcmumps.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libcmumps = "@rpath/libcmumps.dylib"
